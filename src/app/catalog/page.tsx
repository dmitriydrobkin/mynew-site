...
export default function CatalogPage({
  ...
  // Filter products based on category search param
  const filteredProducts = currentCategorySlug
    ? mockProducts.filter((p) => {
        const category = mockCategories.find((c) => c.slug === currentCategorySlug);
        return category ? p.category_id === category.id : false;
      })
    : mockProducts;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-milk">
      <div className="max-w-7xl mx-auto w-full px-4 py-12 md:py-20 flex-grow flex flex-col md:flex-row gap-12">
        ...
        {/* Product Grid Area (to be implemented next step) */}
        <div className="flex-grow">
           <h1 className="text-4xl font-serif text-chocolate mb-8">
             {currentCategorySlug
                ? mockCategories.find(c => c.slug === currentCategorySlug)?.title
                : "Вся коллекция"
             }
           </h1>
